...
data2 <- data[,c(1, 80, 87:101, 136,132,138:139)] %>% 
...
data_diff[c(2:(ncol(data_diff) - 3), ncol(data_diff))] <-
  sapply(
    join_data[, c(2:(ncol(data_diff) - 3), ncol(data_diff))], 
    function(x) {
      diff(x, lag = 1)
  })
...
var.model <- vars::VAR(data_diff[, colnames(data_diff) %in% crudes_to_predict],
                       ...
                       type = "both",
                       season = SEASONALITY,
                       exogen = data_diff[, c(2,18:ncol(data_diff))]
)
...
w_warm <- data.frame(min = weather_pred %>% filter(date %in% misc_pred$date) %>% dplyr::select(upper))
w_cool <- data.frame(min = weather_pred %>% filter(date %in% misc_pred$date) %>% dplyr::select(lower))
...
for (f in f_list) {
  for (w in c(w_avg, w_warm, w_cool)) {
    ...
    exog_reg[[count]] <- cbind(date = misc_pred$date,
                               ...
                               nymex_wti_m1_london_close = misc_pred$nymex_wti_m1_london_close,
                               ice_brent_m1_london_close = misc_pred$ice_brent_m1_london_close,
                               nymex_wti_m1_m2_spread = misc_pred$nymex_wti_m1_m2_spread,
                               ice_brent_m1_m2_spread = misc_pred$ice_brent_m1_m2_spread,
                               f[,-1], 
...
# scaling
centers <- attr(scale(data3[complete.cases(data3) == T,][, c(2, 18:(ncol(data3)-1))]), "scaled:center")
scales <- attr(scale(data3[complete.cases(data3) == T,][, c(2, 18:(ncol(data3)-1))]), "scaled:scale")
# centers; scales
...
exog_reg_diff <- lapply(exog_reg_sc, function(x) {
  x[, c(2:10, 13)] <-
    rbind(rep(NA_real_, 10), diff(as.matrix(x[, c(2:10, 13)]), lag = 1))
  return(x)
})
...
# alternative:
# pred_df2 <- do.call(rbind, pred_list)
  # but copies list elements names into row names
...
pred_df %>% 
  ...
  ggplot() + geom_line(aes((date), price, color = crude)) + 
  facet_grid(weather~financial)
  # facet_grid(financial~weather)
