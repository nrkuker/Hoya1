...
mse <- matrix(nrow = 15, ncol = 10) %>% as.data.frame()
...
for (i in 2:ncol(truth)) {
  rmse_loop <- MSE(pred[-1,i], truth[-1,i])
  var <- colnames(pred)[i]
  # print(paste0("RMSE for ", var, ": ", rmse_loop))
  
  mse[(i-1),count] <- rmse_loop
}

colnames(mse)[count] <- modname

}

rownames(mse) <- colnames(truth)[-1]

# mse
write.csv(mse, str_c(outputname, ".csv"))
